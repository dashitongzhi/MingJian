...
class ChartGenerationService:
    COLORS = ["#4ecdc4", "#ff6b6b", "#ffd93d", "#6c5ce7", "#a8e6cf"]
    BG_COLOR = "#1a1a2e"
    TEXT_COLOR = "#e0e0e0"
    GRID_COLOR = "#333355"
    ...
    def generate_confidence_trajectory(confidence_data: list[dict]) -> str:
        ...
        try:
            ChartGenerationService._setup_style()
    ...
    def generate_argument_comparison(support_args: list, challenge_args: list) -> str:
        ...
        try:
            ...
            legend_elements = [
                Patch(facecolor=ChartGenerationService.COLORS[0], label="支持方"),
                Patch(facecolor=ChartGenerationService.COLORS[1], label="质疑方"),
            ]
