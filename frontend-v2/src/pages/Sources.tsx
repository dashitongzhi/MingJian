import { Database, Link2, RefreshCw, Rss, Signal, Star } from 'lucide-react'
...
export default function Sources() {
  ...
  const { data: custom } = useApi(() => sourcesApi.listCustom())
  const { data: providers } = useApi(() => sourcesApi.listProviders())
  ...
  const customList = asArray(custom)
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">覆盖源游标、自定义源、Provider、声誉、健康、快照和增量变更。</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <MetricCard label="自定义源" value={customList.length} icon={<Rss className="h-5 w-5" />} tone="violet" />
        <MetricCard label="源 Provider" value={providerList.length} icon={<Link2 className="h-5 w-5" />} tone="emerald" />
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_1fr]">
        <Card>
          ...
          {stateList.length === 0 ? <CardBody><EmptyState icon="▥" title="暂无源状态" /></CardBody> : stateList.map((item, index) => <ExpandableRecord key={index} item={item} eyebrow="state" />)}
        </Card>
        <Card>
          <CardHeader title="自定义源与 Provider" />
          {customList.length + providerList.length === 0 ? <CardBody><EmptyState icon="▧" title="暂无自定义源" /></CardBody> : (
            <>
              {customList.map((item, index) => <ExpandableRecord key={`custom-${index}`} item={item} eyebrow="custom" />)}
              {providerList.map((item, index) => <ExpandableRecord key={`provider-${index}`} item={item} eyebrow="provider" />)}
            </>
          )}
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          ...
          {changeList.length === 0 ? <CardBody><EmptyState icon="▦" title="暂无变更" /></CardBody> : changeList.slice(0, 12).map((item, index) => {
              ...
              <div key={id || index} className="border-b border-slate-800/50 px-5 py-4 last:border-b-0">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 line-clamp-2 text-xs text-slate-500">{String(record.summary ?? record.description ?? record.source_key ?? '')}</p>
                  </div>
                  <button disabled={!id || reanalyzing} onClick={() => reanalyze(id).then(reload)} className="rounded-md border border-blue-400/20 bg-blue-500/10 px-3 py-1.5 text-xs text-blue-300 disabled:opacity-50">重分析</button>
                </div>
              </div>
            )
          })}
