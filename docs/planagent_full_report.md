...
- `PATCH /watch/rules/{rule_id}` — 更新规则
- `DELETE /watch/rules/{rule_id}` — 删除规则
- `POST /watch/rules/{rule_id}/trigger` — 立即执行一次规则监控
- `GET /watch/rules/{rule_id}/sources` — 查询规则关联的信源游标状态
- `POST /watch/rules/{rule_id}/cursor/reset` — 重置信源游标并触发重新检查
